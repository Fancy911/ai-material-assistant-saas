...
type ResolveResult = { success: boolean; platform: string; mediaType: 'video' | 'image' | 'gallery' | null; title: string | null; content: string | null; author: string | null; coverUrl: string | null; media: { type: 'video' | 'image'; sourceUrl: string; mimeType: string | null; width: number | null; height: number | null; sizeBytes: number | null }[]; durationSec: number | null; provider: string; rawCode: string | null; errorCode: string | null };
...
export class MockProvider implements ResolverProvider {
  async resolve(platform: string, url: string): Promise<ResolveResult> { return { success: true, platform, mediaType: 'video', title: `Mock ${platform} material`, content: null, author: null, coverUrl: 'https://images.unsplash.com/photo-1492724441997-5dc865305da7', media: [{ type: 'video', sourceUrl: 'https://media.w3.org/2010/05/sintel/trailer.mp4', mimeType: 'video/mp4', width: 1280, height: 720, sizeBytes: null }], durationSec: 52, provider: 'mock', rawCode: 'MOCK_OK', errorCode: null }; }
}
@Injectable()
export class CanxiangProvider implements ResolverProvider {
  async resolve(platform: string, url: string): Promise<ResolveResult> {
    ...
    if (!response.ok || !media.length) return { success: false, platform, mediaType: null, title: null, content: null, author: null, coverUrl: null, media: [], durationSec: null, provider: 'canxiang', rawCode: String(response.status), errorCode: response.status === 401 ? 'PROVIDER_AUTH_ERROR' : 'MEDIA_NOT_FOUND' };
    return { success: true, platform, mediaType: media.length > 1 ? 'gallery' : media[0].type, title: typeof raw.title === 'string' ? raw.title : null, content: firstText(raw.content, raw.desc, raw.description, raw.text), author: null, coverUrl: null, media, durationSec: null, provider: 'canxiang', rawCode: String(response.status), errorCode: null };
  }
}
@Injectable()
export class ZhilingProvider implements ResolverProvider {
  async resolve(platform: string, url: string): Promise<ResolveResult> {
    ...
    const raw = await response.json().catch(() => ({})) as { code?: number; msg?: string; data?: { url?: unknown; videosList?: unknown; title?: unknown; authorName?: unknown; photo?: unknown; picsList?: unknown; content?: unknown; desc?: unknown; description?: unknown; text?: unknown; note?: unknown; caption?: unknown } };
    ...
    return { success, platform, mediaType: !media.length ? null : media.length > 1 ? 'gallery' : media[0].type, title: firstText(data.title), content: firstText(data.content, data.desc, data.description, data.text, data.note, data.caption), author: typeof data.authorName === 'string' ? data.authorName : null, coverUrl: typeof data.photo === 'string' ? data.photo : null, media, durationSec: null, provider: 'zhiling', rawCode: String(raw.code ?? response.status), errorCode: success ? null : raw.code === -1 ? 'PROVIDER_REJECTED' : response.status === 401 || response.status === 403 ? 'PROVIDER_AUTH_ERROR' : 'MEDIA_NOT_FOUND' };
  }
}

function firstText(...values: unknown[]) { return values.find((value): value is string => typeof value === 'string' && value.trim().length > 0)?.trim() ?? null; }
