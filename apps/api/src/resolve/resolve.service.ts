...
export class ResolveService {
  ...
  async getJob(tenantId: string, userId: string, id: string) { const job = await this.prisma.resolveJob.findFirst({ where: { id, tenantId, userId }, include: { media: { select: { id: true, type: true, metaJson: true } } } }); if (!job) throw new ForbiddenException('JOB_NOT_FOUND'); const expiry = Date.now() + Number(process.env.MEDIA_PROXY_TTL_SECONDS || 600) * 1000; const secret = process.env.MEDIA_PROXY_SIGNING_KEY || 'development-only-change-me'; const baseUrl = process.env.APP_PUBLIC_URL || 'http://127.0.0.1:3000'; return { ...job, media: job.media.map((media) => ({ ...media, proxyUrl: `${baseUrl}/api/media/${media.id}?expires=${expiry}&sig=${signMedia(`${media.id}:${userId}:${tenantId}:${expiry}`, secret)}${media.type === 'VIDEO' ? '&format=h264' : ''}` })) }; }
}
