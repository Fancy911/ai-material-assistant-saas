...
describe('ZhilingProvider', () => {
  ...
  it('uses documented GET parameters and maps video, gallery and metadata', async () => {
    global.fetch = jest.fn().mockResolvedValue({ ok: true, status: 200, json: async () => ({ code: 200, data: { title: '测试标题', desc: '这是解析到的正文。', authorName: '作者', photo: 'https://cdn.example.com/cover.jpg', videosList: ['https://cdn.example.com/a.mp4', 'https://cdn.example.com/b.mp4'] } }) });
    ...
    expect(result).toMatchObject({ success: true, provider: 'zhiling', platform: 'kuaishou', mediaType: 'gallery', title: '测试标题', content: '这是解析到的正文。', author: '作者' });
