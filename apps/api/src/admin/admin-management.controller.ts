...
const publicCode = (value: string) => value.trim().toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
...
export class TenantAdminController {
  ...
  async overview(@CurrentSession() session: Session) {
    ...
    const [tenant, users, todayJobs, todaySuccess, newUsers, pointSum, todayRedemptions, capabilityRows, distribution, activation] = await Promise.all([
  ...
  @Get('settings')
  async settings(@CurrentSession() session: Session) { const tenantId = this.tenant(session); const [setting, activation] = await Promise.all([this.prisma.tenantSetting.findUniqueOrThrow({ where: { tenantId } }), this.prisma.tenantActivationCode.findFirst({ where: { tenantId, status: 'ACTIVATED' }, select: { activatedAt: true } })]); return { ...setting, appsecretCiphertext: setting.appsecretCiphertext ? '已配置' : null, wechat: { appid: setting.appid, appsecretConfigured: Boolean(setting.appsecretCiphertext) }, service: { activated: Boolean(activation), activatedAt: activation?.activatedAt ?? null } }; }
  @Patch('settings')
  async updateSettings(@CurrentSession() session: Session, @Body() body: { miniappName?: string; notice?: string | null; initialPoints?: number; pointCost?: number; appsecret?: string; appid?: string; contact?: string; paywallEnabled?: boolean }) {
    ...
    if (body.appid !== undefined) { const appid = body.appid.trim(); if (appid && !/^[a-zA-Z0-9_-]{6,64}$/.test(appid)) throw new BadRequestException('INVALID_WECHAT_APPID'); data.appid = appid || null; } if (body.appsecret !== undefined) { if (body.appsecret && body.appsecret.length < 8) throw new BadRequestException('INVALID_WECHAT_APPSECRET'); data.appsecretCiphertext = body.appsecret ? seal(body.appsecret, secretKey()) : null; }
    const setting = await this.prisma.tenantSetting.update({ where: { tenantId }, data }); await this.audit(session, 'TENANT_SETTINGS_UPDATED', 'tenant_setting', setting.id, { changed: Object.keys(data) }); return { ...setting, appsecretCiphertext: setting.appsecretCiphertext ? '已配置' : null };
  }

  @Post('settings/activate')
  async activateService(@CurrentSession() session: Session, @Body() body: { code?: string }) {
    const tenantId = this.tenant(session); const code = body.code?.trim().toUpperCase();
    if (!code || code.length > 128) throw new BadRequestException('INVALID_ACTIVATION_CODE');
    const activation = await this.prisma.$transaction(async (tx) => {
      const record = await tx.tenantActivationCode.findUnique({ where: { codeHash: codeHash(code) } });
      if (!record || record.tenantId !== tenantId || record.status !== 'UNUSED') throw new BadRequestException('ACTIVATION_CODE_UNAVAILABLE');
      const updated = await tx.tenantActivationCode.updateMany({ where: { id: record.id, tenantId, status: 'UNUSED' }, data: { status: 'ACTIVATED', activatedById: session.sub, activatedAt: new Date() } });
      if (updated.count !== 1) throw new BadRequestException('ACTIVATION_CODE_UNAVAILABLE');
      return tx.tenantActivationCode.findUniqueOrThrow({ where: { id: record.id }, select: { activatedAt: true } });
    });
    await this.audit(session, 'TENANT_SERVICE_ACTIVATED', 'tenant_activation_code', undefined); return { activated: true, activatedAt: activation.activatedAt };
  }
}
...
export class SuperAdminController {
  ...
  @Get('tenants')
  async tenants(@CurrentSession() session: Session, @Query('page') page?: string, @Query('size') size?: string) { this.super(session); const take = takeValue(size); const current = pageValue(page); const [total, items] = await this.prisma.$transaction([this.prisma.tenant.count(), this.prisma.tenant.findMany({ take, skip: (current - 1) * take, orderBy: { createdAt: 'desc' }, include: { _count: { select: { users: true, admins: true, jobs: true } }, capabilities: { where: { enabled: true }, select: { capability: true } }, settings: { select: { appid: true, appsecretCiphertext: true } }, activationCodes: { where: { status: 'ACTIVATED' }, select: { activatedAt: true }, take: 1, orderBy: { activatedAt: 'desc' } } } })]); return { page: current, size: take, total, items: items.map(({ settings, activationCodes, ...tenant }) => ({ ...tenant, wechatConfigured: Boolean(settings?.appid && settings.appsecretCiphertext), activatedAt: activationCodes[0]?.activatedAt ?? null, serviceActivated: Boolean(activationCodes[0]) })) }; }

  @Post('tenants')
  async createTenant(@CurrentSession() session: Session, @Body() body: { name?: string; publicCode?: string; quotaTotal?: number; expiresAt?: string; capabilities?: string[]; adminAccount?: string; adminPassword?: string }) { this.super(session); const name = body.name?.trim(); const tenantPublicCode = publicCode(body.publicCode || `tenant-${randomBytes(3).toString('hex')}`); const quota = Number(body.quotaTotal); if (!name || name.length > 80 || !/^[a-z0-9-]{3,64}$/.test(tenantPublicCode) || !Number.isInteger(quota) || quota < 0 || quota > 10_000_000 || !body.adminAccount?.trim() || !body.adminPassword || body.adminPassword.length < 10) throw new BadRequestException('INVALID_TENANT_CREATE'); const expiresAt = body.expiresAt ? new Date(body.expiresAt) : null; if (expiresAt && Number.isNaN(expiresAt.valueOf())) throw new BadRequestException('INVALID_EXPIRY'); const tenant = await this.prisma.$transaction(async (tx) => { const value = await tx.tenant.create({ data: { name, publicCode: tenantPublicCode, quotaTotal: quota, quotaRemaining: quota, expiresAt } }); await tx.tenantSetting.create({ data: { tenantId: value.id } }); await tx.tenantAdmin.create({ data: { tenantId: value.id, account: body.adminAccount!.trim(), passwordHash: await argon2.hash(body.adminPassword!), role: 'TENANT_ADMIN' } }); for (const capability of [...new Set(body.capabilities || [])]) await tx.tenantCapability.create({ data: { tenantId: value.id, capability, enabled: true } }); return value; }); await this.audit(session, 'TENANT_CREATED', 'tenant', tenant.id, { name, publicCode: tenantPublicCode, quota, capabilities: body.capabilities || [] }); return tenant; }

  @Post('tenants/:id/activation-codes')
  async createActivationCode(@CurrentSession() session: Session, @Param('id') tenantId: string) {
    this.super(session); const tenant = await this.prisma.tenant.findUnique({ where: { id: tenantId }, select: { id: true } }); if (!tenant) throw new BadRequestException('TENANT_NOT_FOUND');
    const code = `BXY-ACT-${randomBytes(5).toString('hex').toUpperCase()}-${randomBytes(2).toString('hex').toUpperCase()}`;
    const activation = await this.prisma.$transaction(async (tx) => { await tx.tenantActivationCode.updateMany({ where: { tenantId, status: 'UNUSED' }, data: { status: 'REVOKED' } }); return tx.tenantActivationCode.create({ data: { tenantId, codeHash: codeHash(code), codeHint: `${code.slice(0, 8)}••${code.slice(-4)}`, createdById: session.sub } }); });
    await this.audit(session, 'TENANT_ACTIVATION_CODE_CREATED', 'tenant_activation_code', activation.id, { tenantId, hint: activation.codeHint }); return { code, hint: activation.codeHint, createdAt: activation.createdAt };
  }
