...
export async function ensureResolvedPublicHost(input: string) { const url = ensureSafeMediaUrl(input); const addresses = await lookup(url.hostname, { all: true, verbatim: true }); if (!addresses.length || addresses.some(({ address }) => isUnsafeHost(address))) throw new BadRequestException('Unsafe media host'); return url; }
