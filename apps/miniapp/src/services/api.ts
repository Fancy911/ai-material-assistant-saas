...
export async function ensureSession(force = false) {
  ...
  const result = await request<{ accessToken: string }>('/api/auth/wechat-login', 'POST', { code, tenantCode }, false); uni.setStorageSync('access_token', result.accessToken); return result.accessToken;
}
...
export async function getMe() { await ensureSession(); return request<{ pointsBalance: number; totalResolves: number; todayResolves: number; pointCost: number; miniappName: string; notice: string | null; service: { activated: boolean; canResolve: boolean; tenantStatus: string } }>('/api/me'); }
