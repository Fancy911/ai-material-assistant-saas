...
export function detectPlatform(input: string) { const url = extractUrl(input); const host = url?.match(/^https?:\/\/([^/?#:]+)/i)?.[1]?.toLowerCase(); if (!host) return null; return platformRules.find(([, pattern]) => pattern.test(host))?.[0] ?? 'generic'; }
